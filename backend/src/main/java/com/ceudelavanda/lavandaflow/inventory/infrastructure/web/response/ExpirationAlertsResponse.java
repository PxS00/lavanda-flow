...
public record ExpirationAlertsResponse(
    @Schema(
        description = "Application date used to evaluate all expiration states using the configured application Clock.",
        example = "2026-08-26"
    )
    LocalDate asOfDate,
    @Schema(
        description = "Resolved non-negative future alert window in days. This is the request override when supplied, otherwise the configured default.",
        minimum = "0",
        example = "30"
    )
    int windowDays,
    @Schema(
        description = "Expired and expiring-soon positive-balance batches, ordered by expiration date, inventory item ID, and batch ID."
    )
