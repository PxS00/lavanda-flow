...
public record LowStockAlertsResponse(
    @Schema(description = "Business date used to determine available stock.", example = "2026-08-26") LocalDate asOfDate,
    ...
    public static LowStockAlertsResponse from(LowStockAlertsResult result) {
        return new LowStockAlertsResponse(
            result.asOfDate(), result.alerts().stream().map(LowStockAlertEntryResponse::from).toList()
        );
    }
}
