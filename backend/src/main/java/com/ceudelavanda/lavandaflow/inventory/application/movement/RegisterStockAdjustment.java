...
/**
 * Application use case responsible for registering signed adjustments for an explicitly selected inventory batch.
 *
 * <p>The balance correction and immutable audit movement are persisted atomically. The selected batch is
 * pessimistically locked before its balance is loaded so concurrent mutations are serialized.</p>
 */
@Service
...
public class RegisterStockAdjustment {
     ...
     *
     * @param command adjustment data, including the selected batch and mandatory audit reason
     * @return the persisted movement data and resulting batch balance
     * @throws BatchNotFoundException if the batch does not exist
     * @throws InvalidStockAdjustmentException if the reason is blank or the adjustment is zero
     * @throws InsufficientStockException if a negative adjustment exceeds the available balance
     * @throws IllegalArgumentException if quantity precision or other domain invariants are violated
     */
    @Transactional
    public StockMovementResult execute(RegisterStockAdjustmentCommand command) {
        ...
        var movement = StockMovement.create(
            batch.getId(), movementType, command.quantity().abs(), command.reason(), Instant.now(clock)
        );
    ...
    private static void requireReason(String reason) {
        if (reason == null || reason.isBlank()) {
            throw new InvalidStockAdjustmentException("Stock adjustment reason must not be blank");
        }
    }
}
