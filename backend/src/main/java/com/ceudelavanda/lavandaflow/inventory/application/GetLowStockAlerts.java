 ...
 *
 * <p>Availability is evaluated at the date obtained from the injected clock. It includes only positive
 * batch balances that either do not expire or expire after that date; expired batches and batches expiring
 * on that date are unavailable. Configured inactive items remain persisted but do not generate alerts.</p>
 */
@Service
...
public class GetLowStockAlerts {
    ...
    private static java.util.Optional<LowStockAlertEntryResult> toAlert(
        ...
        return java.util.Optional.of(new LowStockAlertEntryResult(
            ...
            level.getMinimumQuantity().subtract(availableQuantity)
        ));
    }
}
