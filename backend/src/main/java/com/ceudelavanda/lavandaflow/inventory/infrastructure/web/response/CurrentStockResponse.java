...
public record CurrentStockResponse(
    ...
    boolean active,
    @Schema(
        description = "Total physical/accounting balance across all batches for the item, including expired and zero-balance batches.",
        example = "180.500000"
    )
    ...
    public static CurrentStockResponse from(CurrentStockResult result) {
        return new CurrentStockResponse(
            ...
            result.active(),
            result.totalCurrentQuantity(),
            result.batches().stream()
                .map(BatchStockResponse::from)
                .toList()
        );
    }
}
