...
public record FefoWithdrawalAllocationResponse(UUID batchId, UUID movementId, BigDecimal quantity) {
    static FefoWithdrawalAllocationResponse from(FefoWithdrawalAllocationResult result) {
        return new FefoWithdrawalAllocationResponse(result.batchId(), result.movementId(), result.quantity());
    }
}
